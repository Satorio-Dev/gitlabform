...
class BranchProtection:
    ...
    def map_config_to_protected_branch_get_data(our_branch_config: dict):
        ...
        'protected' and 'additive' are gitlabform's own keys rather than GitLab
        attributes, and are dropped here so that they take no part in a comparison
        against the GET response and never reach a request.
        ...
        new_branch_config.pop("additive", None)
        return new_branch_config

    @staticmethod
    def build_patch_request_data(
        transformed_access_levels: list[dict] | None,
        ...
        A declared access list is the whole of that list. A record GitLab holds that no
        configured rule claims is sent to be destroyed, so that reading the configuration
        is enough to know who may push to, merge into or unprotect the branch. A branch
        carrying "additive: true" keeps the older behaviour instead, where such a record
        is left where it is.

        A list the configuration does not declare at all is a different matter: nothing
        is sent for it and GitLab keeps whatever it holds.
        ...
        args:
            ...
            additive (bool): whether to leave a record the configuration does not name where it is
    ...
    @staticmethod
    def _records_to_destroy(unclaimed_records: list[dict], configured_rules: list[dict], additive: bool) -> list[dict]:
        """The records to send for deletion, out of those no configured rule claimed.

        All of them: a declared list is the whole of that list, so what GitLab holds and
        the configuration does not name goes.

        Under "additive: true" a record the configuration does not name is left where it
        ...
        """
        if not additive:
            return BranchProtection._destroy_entries(unclaimed_records)
    ...
    def naive_access_level_diff_analyzer(_, cfg_in_gitlab: list, local_cfg: list):
        ...
        An update is needed if any rule the local configuration asks for is missing from
        GitLab, or if the two sides disagree about "No Access", which is exclusive with
        every other role.

        This answers only what the configuration asks for and GitLab lacks. What GitLab
        holds and the configuration does not name is a separate question, and one the
        PATCH payload answers by itself - see build_patch_request_data.
        """
